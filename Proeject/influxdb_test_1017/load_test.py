...
# IC.load_stock_data_API(stock='NIO', start_time=datetime.strptime("2021-10-19", '%Y-%m-%d'),
#                        stop_time=datetime.strptime("2022-10-19", '%Y-%m-%d'))

IC.get_stock_data(stock='NIO', start_time=datetime.strptime("2021-10-19", '%Y-%m-%d'),
                       stop_time=datetime.strptime("2022-10-19", '%Y-%m-%d'))
...
for stock in test_stocks:
    print(stock)
    IC.load_stock_data_API(stock=stock, start_time=start_time, stop_time=stop_time)


start_time=datetime.strptime("2022-03-19", '%Y-%m-%d')
stop_time=datetime.strptime("2022-10-19", '%Y-%m-%d')
for stock in test_stocks:
    print(stock)
    print(IC.get_stock_data(stock=stock, start_time=start_time,
                       stop_time=stop_time))
...
# query_client = _client.query_api()
#     data_frame = query_client.query_data_frame('from(bucket:"433db")'
#                                                '|> range(start: -1m) '
#                                                '|> filter(fn: (r) => r["_measurement"] == "NIO")'
#                                                '|> pivot(rowKey:["_time"], columnKey: ["_field"], valueColumn: "_value") '
#                                                '|> keep(columns: ["_measurement","_time","open", "high", "low", "close","volume", "trade_count","vwap"])')
#     print(data_frame.to_string())
#     print("success")